...
class Hunk:
    ...
    def content_preview(self, max_lines: int = 3) -> list[str]:
        added = [l[1:] for l in self.lines if l.startswith("+")]
        return [l.strip() for l in added[:max_lines]]
...
def cmd_reconstruct(args: argparse.Namespace) -> None:
    ...
    # Use the first file's hunks (the diff should be for a single file)
    file_diff = files[0]

    # Parse hunk indices to apply
    if args.hunks:
        selected_indices = set(int(x) for x in args.hunks.split(","))
    else:
...
def apply_hunks(base_lines: list[str], hunks: list[Hunk]) -> list[str]:
    ...
    Hunks must be in order of their old_start position.
    ...
    for hunk in sorted_hunks:
        ...
        while base_idx < hunk_start and base_idx < len(base_lines):
